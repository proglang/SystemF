...
(# n)     ⋯ᵣ ρ = # n
(` x)     ⋯ᵣ ρ = ` (go x ρ)
  where go : {t : Δ₁ ⊢ l} → {ρ : T.Ren Δ₁ Δ₂} → Γ₁ ∍ t → Ren ρ Γ₁ Γ₂ → Γ₂ ∍ (t T.⋯ᵣ ρ)
        go x          id        = subst (_ ∍_) (sym (⋯ᵣ-id _)) x
        go here      (↑ ρ)      = here
        go (there x) (↑ ρ)      = there (go x ρ)
        go (skip x)  (↑ₗ ρ)     = subst (_ ∍_) (wkᵣ-↑ᵣ _ _) (skip (go x ρ))
        go x         (wk ρ)     = subst (_ ∍_) (⋯ᵣᵣ-fusion _ _ _) (skip (go x  ρ))
        go x         (⋯· ρ₁ ρ₂) = subst (_ ∍_) (⋯ᵣᵣ-fusion _ _ _) (go (go x ρ₁) ρ₂)
(λx e)    ⋯ᵣ ρ = λx (e ⋯ᵣ ↑ ρ)
...
(# n)        ⋯ₛ σ = # n
(` x)        ⋯ₛ σ    = ` (go x σ)
  where go : {t : Δ₁ ⊢ l} → {σ : T.Sub Δ₁ Δ₂} → Γ₁ ∍ t → Sub σ Γ₁ Γ₂ → Γ₂ ∍ (t T.⋯ₛ σ)
        go x                id      = subst (_ ∍_) (sym (⋯ₛ-id _)) x
        go here             (↑ σ)   = here
        go (there x)        (↑ σ)   = there (go x σ)
        go (skip {t = t} x) (↑ₗ σ)  = subst (_ ∍_) (wkᵣ-↑ₛ t _) (skip (go x σ))
        go (skip {t = t} x) (ext σ) = subst (_ ∍_) (sym (wkᵣ-cancels-extₛ t _ _)) (go x σ)
(λx e)       ⋯ₛ σ = λx (e ⋯ₛ ↑ σ)
