...
\begin{enumerate}
\item the equation between line \ACom{(4)} and \ACom{(5)} states that if
...
\item the equation between line \ACom{(5)} and \ACom{(6)} requires another
  use of {\AFun{dist-subst}} to pull {\AFun{S₆}} out of the left
  argument of the type application, which introduces {\AFun{S₈}};
\item the equation between line \ACom{(6)} and \ACom{(7)} is where
...
Equipped with these lemmas, we turn back to the example of the fusion lemma for substitutions.
