...
Consider for example the type application case of the lemma for
expression substitution from Section~\ref{sec:substitutions:expr}:
...
For the sake of readability we abstracted the {\Asubst} terms.
Each \ACode{Sᵢ} stands for a \ACode{\Asubst~Fᵢ~Eᵢ}.
As the concrete arguments of the \ACode{Sᵢ} are not relevant to the
discussion, we elided the \AKw{where}-block of the proof.

In line (1), the \ACode{S₁} comes from the lemma statement itself.
In line (2), the inner {\AEsub} from line (1) has been unfolded, which
introduced another {\Asubst} we call \ACode{S₂}. The \ACode{S₂} blocks
the unfolding of the outer {\AEsub}. It is impossible to get \ACode{S₂} to reduce,
because \ACode{E₂} is an equality proof between two function
applications, so it cannot be matched on. However,
the \ACode{S₂} can be pulled outwards, which transforms it into
\ACode{S₃} as shown in line (3) and unblocks the outer {\AEsub}.
This is possible because {\AEsub} is sufficiently polymorphic, such that
it is welltyped when called both with and without \ACode{S₂} -- it just produces a value
of a different type, which needs to be accounted for with \ACode{S₃}.
