...
Consider for example the type application case {\ACon{\_∙\_}} of the fusion lemma for
...
As the concrete arguments of the {\AFun{Sᵢ}} are not relevant to the
discussion, we elided the \AKw{where}-block of the proof.

In line \ACom{(1)}, the {\AFun{S₁}} is the \ACode{S} from the lemma statement itself.
...
it is welltyped when called both with and without {\AFun{S₂}} -- it just produces values
...
This is exactly what the {\AFun{dist-subst}} lemma does:
\SubstExamplesDistSubst
To prove the equation required to go from line \ACom{(2)} to \ACom{(3)}, we
instantiate {\AFun{dist-subst}} as followed:
...
The {\AFun{cong₂}} lemma for heterogeneous equality, allows to lift
...
Consequently, in line \ACom{(1)} there is also no {\AFun{S₁}}.
However, in line \ACom{(2)}, \ACom{(4)} and \ACom{(7)}, we still get the {\AFun{S₂}} from reducing the {\AESub}.
This is unavoidable as {\AESub} is a definition, and hence needs to
produce a term of exactly the type it is checked against.
In line \ACom{(4)} and \ACom{(7)}, we can use the {\AFun{≡-subst-removable}} lemma
directly to remove {\AFun{S₄}} and {\AFun{S₅}}, respectively:
\SubstExamplesFusionESubESubHetProofB

In line \ACom{(2)}, the application of {\AFun{S₂}} occurs not on the outside but as an argument to
...
This is not the case, because a unary {\AFun{cong}} can only replace values of the \emph{same} type.
...
In line \ACom{(5)} we have a similar situation where want to use the
...
binary {\AFun{cong₂}}, we need a tertiary {\AFun{cong₃}} to account
...
using {\Asubst} in definitions and lemma statements. While it completely removes the need for using
{\Asubst} in lemma statements, some {\Asubst}
applications still show up in equality proofs due to definitions, where subst
cannot be avoided in an intrinsically typed setting.
These {\Asubst} applications need to be removed manually, which with heterogeneous
equality behaves much more regular than with homogeneous equality. In
particular, we started our formalization with homogenous equality,
which required us to prove around 20 lemmas in the style of
\AFun{dist-subst}. Switching to heterogeneous equality, allowed us to
