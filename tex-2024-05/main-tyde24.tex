...
% Drawback: \textemdash{} has to be used instead of --- in the whole Latex document...
...
operational semantics based on intrinsically typed syntax. While its syntactic
...
\keywords{mechanized metatheory, finitely-stratified System F,
  denotational semantics, operational semantics, Agda}
...
\item We construct a denotational semantics of $SF_2$ as a
  ...
  define big-step (Sec.~\ref{sec:big-step-operational})
  and prove its soundness with respect to the denotational semantics.
\item We define a logical relation for $SF_2$ that connects the
  denotational semantics with the big-step semantics.
  We establish the fundamental
...
\item All of the above is implemented in Agda
  \cite{norell07:_towar_pract_progr_languag_depen_type_theor} and
    available online: \url{https://github.com/proglang/SystemF}
\end{itemize}
...
prove similar results about type substitutions as we do, but they do not
...
Type abstraction \texttt{\ACon{Λ} $l$ \ACon{⇒} $e$} takes a level $l$
...
expression-in-expression substitution (marked with \AgdaFunction{\_[\_]E}) and type-in-expression
substitution (marked with \AgdaFunction{\_[\_]ET}), respectively.
...
relation {\AGSemNew} on environments.
...
The relation {\AGSemNew} extends {\AVSem} to relate closing substitutions
...
The fundamental theorem of the logical relation states that well-typed
expressions are semantically sound.
...
from {\AGSemNew} and construct the result from $w$, the proof that $w$
...
environment of type \ACode{\AFun{Env} \ACon{[]} {\Anull} \ACon{[]}}.
...
it is well-typed when called both with and without {\AFun{S₂}} \textemdash{} it just produces values
...
\item the equation for {\AFun{p₃}} between line \ACom{(5)} and \ACom{(6)} requires another
  use of {\AFun{dist-subst}} to pull out {\AFun{S₆}} of the left
  argument of the type application, introducing {\AFun{S₈}};
\item the equation for {\AFun{p₄}} between line \ACom{(6)} and \ACom{(7)} is where
  ...
  whole type application, so a specialized {\AFun{cong}} lemma is required, which exchanges
...
In contrast to homogeneous equality \AgdaDatatype{\_≡\_}, heterogenous
...
\ACode{\{x~:~A\}~→~(y~:~B~x)~→~C~x~y}, where the argument types are two different instances
of the type family \ACode{B}. In our case the partially applied function \ACode{\AFun{Esub}~σ₁*~σ₁}
has type\\
\ACode{\{T~:~\AFun{Type}~Δ₂~l\}~→~\AFun{Expr}~Δ₂~Γ₂~T~→~\AFun{Expr}~Δ₃~Γ₃~(\AFun{Tsub}~σ₁*~T)}\\
and {\AFun{S₂}} exchanges the implicit type index \ACode{T} with
some \ACode{T′} with \ACode{T \AFun{≡} T′}.
Consequently, we can prove the equation with {\AFun{cong₂}}:
\SubstExamplesFusionESubESubHetProofA
% The first two arguments of {\AFun{cong₂}} are the \ACode{A} and
% \ACode{B} as described above. The third argument specifies the binary
% function. The fourth argument is the equality on the implicit type
% index of \ACode{\AFun{Esub}~σ₁*~σ₁}. The fifth argument 


% Alas, a unary {\AFun{cong}} can only replace values of the \emph{same} type.
% To be able to call a function like \ACode{\AFun{Esub}~σ₁*~σ₁} with arguments of two different types,
...
% Consider for example the type \texttt{Vec} \textemdash{} a list that
% intrinsically tracks its length \textemdash{} and its append function \verb/_++_/:
% \SubstExamplesVec
% \SubstExamplesAppend
...
% indexed by a type-substitution \textemdash{} in this case \texttt{Tid} \textemdash{} we need
